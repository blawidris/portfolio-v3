...
export async function POST(
  ...
  try {
    ...
    if (!existing) return apiError(404, "NOT_FOUND", "The requested project was not found.")

    const previewToken = randomBytes(24).toString("hex")
    await prisma.project.update({ where: { id }, data: { previewToken } })
    return Response.json({ previewToken })
  } catch (error) {
    return handleApiError(error, "projects.preview-token")
  }
}
