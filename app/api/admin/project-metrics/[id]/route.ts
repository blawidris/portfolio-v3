...
export async function DELETE(
  ...
  try {
    ...
    const existing = await prisma.projectMetric.findUnique({ where: { id }, include: { project: { select: { slug: true } } } })
    if (!existing) return apiError(404, "NOT_FOUND", "The requested record was not found.")

    await prisma.projectMetric.delete({ where: { id } })
    revalidateProjectContent(existing.project.slug)
    return new Response(null, { status: 204 })
  } catch (error) {
    return handleApiError(error, "project-metrics.delete")
  }
}
