...
export default async function AdminDashboard() {
  let stats = { totalPosts: 0, publishedPosts: 0, draftPosts: 0, totalProjects: 0, featuredProjects: 0, footerLinks: 0 }
  let recentActivity: { type: "post" | "project"; id: string; title: string; updatedAt: Date }[] = []
  ...
  try {
    ...
    recentActivity = [
      ...posts.slice(0, 3).map((p) => ({ type: "post" as const, id: p.id, title: p.title, updatedAt: p.updatedAt })),
      ...projects.slice(0, 2).map((p) => ({ type: "project" as const, id: p.id, title: p.title, updatedAt: p.updatedAt })),
    ]
