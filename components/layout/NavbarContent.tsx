...
export default function NavbarContent({ navLinks }: { navLinks: NavLink[] }) {
  ...
  return (
    ...
    >
      ...
      {menuOpen && (
        <div id="mobile-navigation" className="md:hidden bg-[var(--bg-secondary)] border-b border-[var(--border)] px-6 pb-6 pt-2">
          {navLinks.map(({ href, label }) => (
            <Link
              key={href}
              href={href}
